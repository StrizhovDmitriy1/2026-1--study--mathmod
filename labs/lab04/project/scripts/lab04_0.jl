# ## Инициализация проекта
using DrWatson 
@quickactivate "project"
using DifferentialEquations
using Plots 
default(fmt = :png)


# ## 1. Параметры модели (Вариант 54, №1)
w0 = sqrt(9.9)
g = 0 # Система без затухания 
x0 = [0.9, 0.9]
tspan = (0.0, 48)
f(t) = 0 # Система без действия внешних сил

# ## 2. Определение системы
function oscillator!(du, u, p, t)
	x, y = u
	du[1] = y
	du[2] = -w0^2 * x - g * y - f(t)
end

# ## 3. Решение системы и построение графика
prob1 = ODEProblem(oscillator!, x0, tspan)
sol1 = solve(prob1, Tsit5(), saveat = 0.05)

p1 = plot(sol1, title="Решение: без затухания", xlabel="t", label=["x(t)" "y(t)"])
p2 = plot(sol1, vars=(1,2), title="Фазовый портрет", xlabel="x", ylabel="y", label="Траектория")
final_plot = plot(p1, p2, layout=(1,2))

display(final_plot)

# Сохранение 
script_name = "lab04"
mkpath(plotsdir(script_name))

savefig(final_plot, plotsdir(script_name, "oscillator_simple.png"))
